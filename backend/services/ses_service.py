...
Palette: fond #F4F0E8 · violet #A65D47 · or #C9A84C
"""
import os
import io
import logging
...
def _generate_qr_base64(data: str, size: int = 200) -> str:
    ...
    qr.add_data(data)
    qr.make(fit=True)
    img = qr.make_image(fill_color="#A65D47", back_color="#ffffff")
    buf = io.BytesIO()
...
<body style="margin:0;padding:0;background:#F4F0E8;font-family:'Segoe UI',Roboto,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" style="background:#F4F0E8;">
<tr><td align="center" style="padding:20px 0;">
<table width="600" cellpadding="0" cellspacing="0" style="background:#FFFFFF;border-radius:12px;overflow:hidden;border:1px solid #E8E0D0;">
<!-- Header -->
<tr><td style="background:linear-gradient(135deg,#A65D47,#C9A84C);padding:30px;text-align:center;">
  <h1 style="color:#FFFFFF;margin:0;font-size:24px;letter-spacing:2px;">CULTURE CONNECT 2026</h1>
  <p style="color:rgba(255,255,255,0.8);margin:8px 0 0;font-size:13px;">22 Mai 2026 · Parc de La Savane · Fort-de-France</p>
</td></tr>
<!-- Content -->
<tr><td style="padding:30px;color:#1A1510;line-height:1.7;font-size:15px;">
{content}
</td></tr>
<!-- Footer -->
<tr><td style="background:#F4F0E8;padding:20px 30px;text-align:center;border-top:1px solid #E8E0D0;">
  <p style="color:#A65D47;margin:0 0 5px;font-size:12px;">Factory Maker Studio (EURL) · CVLN Group</p>
  <p style="color:#6B6560;margin:0;font-size:11px;">kiltikonet.fr · cc@kiltikonet.fr</p>
</td></tr>
</table>
</td></tr></table>
</body></html>"""
...
<h2 style="color:#A65D47;margin:0 0 15px;">Bienvenue {prenom} !</h2>
<p>Votre inscription a bien ete enregistree pour <strong style="color:#A65D47;">Culture Connect 2026</strong>.</p>
<div style="background:#F4F0E8;border:1px solid #A65D47;border-radius:8px;padding:20px;margin:20px 0;text-align:center;">
  ...
  <p style="color:#6B6560;margin:0 0 15px;font-size:12px;">FREK-ID: {frek_id}</p>
  <img src="data:image/png;base64,{qr_b64}" alt="QR Code" style="width:160px;height:160px;border-radius:8px;"/>
  <p style="color:#6B6560;margin:10px 0 0;font-size:11px;">Scannez pour activer votre badge</p>
</div>
<a href="{APP_URL}/activer-badge/{qr_token}" style="display:inline-block;background:#C9A84C;color:#F4F0E8;padding:12px 30px;border-radius:6px;text-decoration:none;font-weight:bold;margin:10px 0;">
  Activer mon Badge
</a>
<p style="color:#6B6560;font-size:13px;margin-top:20px;">Conservez cet email — il contient votre QR code d'acces.</p>
""")


def template_wallet_recharge(prenom: str, badge_id: str, pack_name: str, jetons_ajoutes: int, nouveau_solde: int) -> str:
    return _wrap_template(f"""
<h2 style="color:#A65D47;margin:0 0 15px;">Rechargement confirme !</h2>
<p>Bonjour {prenom},</p>
<p>Votre porte-monnaie Culture Connect a bien ete recharge.</p>
<div style="background:#F4F0E8;border:1px solid #A65D47;border-radius:8px;padding:20px;margin:20px 0;text-align:center;">
  <p style="color:#6B6560;margin:0 0 5px;font-size:12px;">PACK {pack_name.upper()}</p>
  <p style="color:#A65D47;font-size:36px;font-weight:bold;margin:0;">+{jetons_ajoutes} Jetons</p>
  <hr style="border:none;border-top:1px solid #A65D47;margin:15px 0;"/>
  <p style="color:#6B6560;margin:0 0 5px;font-size:12px;">SOLDE ACTUEL</p>
  <p style="color:#fff;font-size:28px;font-weight:bold;margin:0;">{nouveau_solde} Jetons CC</p>
</div>
<p>Badge: <strong style="color:#A65D47;">{badge_id}</strong></p>
<p style="color:#6B6560;font-size:13px;">1 Jeton CC = 1.50EUR de valeur faciale sur l'evenement.</p>
""")


def template_rappel_j15(prenom: str, badge_id: str) -> str:
    return _wrap_template(f"""
<h2 style="color:#A65D47;margin:0 0 15px;">J-15 — On vous attend !</h2>
<p>Bonjour {prenom},</p>
<p>Plus que <strong style="color:#A65D47;">15 jours</strong> avant Culture Connect 2026 !</p>
<div style="background:#F4F0E8;border:1px solid #A65D47;border-radius:8px;padding:20px;margin:20px 0;">
  <p style="margin:0 0 10px;">Votre badge <strong style="color:#A65D47;">{badge_id}</strong> est pret.</p>
  <p style="margin:0 0 10px;">Pensez a recharger vos Jetons CC pour profiter de toutes les experiences du Parc de La Savane.</p>
</div>
<a href="{APP_URL}/jetons" style="display:inline-block;background:#C9A84C;color:#F4F0E8;padding:12px 30px;border-radius:6px;text-decoration:none;font-weight:bold;">
...
def template_rappel_j1(prenom: str, badge_id: str) -> str:
    return _wrap_template(f"""
<h2 style="color:#A65D47;margin:0 0 15px;">C'est demain !</h2>
<p>Bonjour {prenom},</p>
<p><strong style="color:#A65D47;">Culture Connect 2026</strong> c'est demain, 22 Mai !</p>
<div style="background:#F4F0E8;border:1px solid #A65D47;border-radius:8px;padding:20px;margin:20px 0;">
  <p style="margin:0 0 10px;">Votre badge: <strong style="color:#A65D47;">{badge_id}</strong></p>
...
<p style="color:#6B6560;font-size:13px;">N'oubliez pas votre QR code (email de bienvenue) !</p>
""")


def template_jour_j(prenom: str, badge_id: str) -> str:
    return _wrap_template(f"""
<h2 style="color:#A65D47;margin:0 0 15px;">Jour J — Bienvenue !</h2>
<p>Bonjour {prenom},</p>
<p style="font-size:18px;"><strong style="color:#A65D47;">Culture Connect 2026</strong> commence maintenant !</p>
<div style="background:#F4F0E8;border:1px solid #A65D47;border-radius:8px;padding:20px;margin:20px 0;">
  <p style="margin:0 0 10px;">Badge: <strong style="color:#A65D47;">{badge_id}</strong></p>
...
def template_merci_j1(prenom: str, badge_id: str) -> str:
    return _wrap_template(f"""
<h2 style="color:#A65D47;margin:0 0 15px;">Merci {prenom} !</h2>
<p>Culture Connect 2026 s'est acheve, et vous en avez fait partie.</p>
<div style="background:#F4F0E8;border:1px solid #A65D47;border-radius:8px;padding:20px;margin:20px 0;">
  <p style="margin:0 0 10px;">Votre badge <strong style="color:#A65D47;">{badge_id}</strong> reste actif.</p>
...
<p style="color:#A65D47;font-weight:bold;">— L'equipe Culture Connect</p>
""")
...
<div style="background:#2a0000;border:1px solid #ff4444;border-radius:8px;padding:20px;margin:20px 0;">
  ...
  <p style="margin:0;color:#1A1510;">{details}</p>
  <p style="margin:10px 0 0;color:#6B6560;font-size:12px;">{datetime.now(timezone.utc).isoformat()}</p>
</div>
""")
...
async def send_badge_recovery(to_email: str, prenom: str, badge_id: str, frek_id: str, qr_token: str) -> dict:
    ...
    return await _send_ses_email(to_email, f"Recuperation badge — Culture Connect 2026", html)
