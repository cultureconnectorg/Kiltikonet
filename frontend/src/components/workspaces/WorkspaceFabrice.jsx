...
import { LogOut, Monitor, Send, Play, Pause, Edit2, Plus, Clock, RefreshCw, Camera, Upload, CheckSquare, Square, AlertTriangle, Radio, Image } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>CFA M.A.N.S - Régie Live</div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold ${isLive ? 'animate-pulse' : ''}`}
              style={{ background: isLive ? 'rgba(239,68,68,0.2)' : 'rgba(255,255,255,0.1)', color: isLive ? COLORS.red : 'rgba(255,255,255,0.4)' }}>
              <div className={`w-3 h-3 rounded-full ${isLive ? 'bg-red-500' : 'bg-white/30'}`} />
              {isLive ? 'EN DIRECT' : 'HORS LIGNE'}
            </div>
            {!isLive ? (
              <Button onClick={activateLive} style={{ background: COLORS.red }}>
                <Play className="w-4 h-4 mr-2" />
                Activer Live
              </Button>
            ) : (
              <Button onClick={deactivateLive} variant="outline" style={{ borderColor: COLORS.red, color: COLORS.red }}>
