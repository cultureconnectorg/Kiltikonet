...
export default function WalletView({ onBack, onSelect, balance, setBalance, transactions, addTransaction }) {
  ...
  const handleTopUp = async (amount) => {
    ...
    setTimeout(() => {
      ...
      addTransaction({ type: "receive", label: "Dépôt Instantané", amount: `+${amount} JCC` });
  ...
  const assets = [
    { name: "Jeton CC", symbol: "JCC", balance: balance, value: `${(balance * 0.85).toFixed(2)}€`, color: "text-[#f2ca50]" },
    ...
    <div className="flex flex-col h-screen w-full text-white overflow-hidden relative" style={{ background: '#050505' }} data-testid="wallet-view">
        ...
        <div className="max-w-md mx-auto space-y-4">
            ...
            <div className="relative z-10 h-full flex flex-col justify-between">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-[10px] text-gray-500 font-mono tracking-wider">≈ {(balance * 0.85).toFixed(2)} EUR</span>
              ...
              <div className="flex justify-between items-end">
                <div className="flex flex-col">
                  ...
                  <span className="text-[9px] font-mono text-white/50 tracking-wider">99421-MQ-****</span>
                </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              { label: "TOP UP", icon: Plus, bg: "", action: () => handleTopUp(50), special: true }
            ].map((a, i) => {
              const Icon = a.icon;
              return (
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 300 }} className="w-full max-w-md rounded-t-[2rem] sm:rounded-[2rem] p-6 space-y-6" style={{ background: '#111', border: '1px solid rgba(255,255,255,0.1)' }} onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center">
                <h3 className="italic text-xl uppercase tracking-wider" style={{ fontFamily: "'Noto Serif', serif", color: '#f2ca50' }}>
                  {activeModal === "send" && "Envoyer JCC"}{activeModal === "receive" && "Recevoir JCC"}{activeModal === "swap" && "Swap JCC"}
                </h3>
              ...
              {activeModal === "receive" && (
                <div className="flex flex-col items-center space-y-6 py-4">
                  ...
                  <div className="text-center space-y-2">
                    ...
                    <div className="bg-white/5 rounded-xl p-3 font-mono text-[10px] select-all" style={{ border: '1px solid rgba(255,255,255,0.1)', color: '#f2ca50' }}>99421-MQ-SVR-ALCH</div>
                  </div>
                </div>
              )}
              {activeModal === "swap" && (
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl" style={{ border: '1px solid rgba(255,255,255,0.1)' }}>
                    ...
                    <span className="text-sm font-bold" style={{ color: '#f2ca50' }}>{(parseFloat(swapAmount || "0") * 0.85).toFixed(2)}€</span>
                  </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 z-50" style={{ background: 'linear-gradient(to top, black, rgba(0,0,0,0.8), transparent)' }}>
        <div className="max-w-md mx-auto">
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleTopUp(100)} disabled={isTopUpLoading}
            ...
            {isTopUpLoading ? "Processing..." : "Instant Top Up (Stripe)"}
          </motion.button>
        </div>
      </div>
    </div>
