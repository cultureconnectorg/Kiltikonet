...
const FEED_DATA = [
  { id: 1, user: "Ben ARRIS", avatar: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=100", content: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=1920", description: "Synchronizing the alchemy of digital gestures with institutional precision.", likes: "42.8K", comments: "1.2K", category: "BRUT", location: "GHANA" },
  { id: 2, user: "Kilti Maker", avatar: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&q=80&w=100", content: "https://images.unsplash.com/photo-1598488035139-bdbb2231ce04?auto=format&fit=crop&q=80&w=1920", description: "New session studio — Beat Zouk available now in the shop.", likes: "12.5K", comments: "450", category: "MUSIC", location: "MARTINIQUE" },
  { id: 3, user: "Core Engine", avatar: "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&q=80&w=100", content: "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&q=80&w=1920", description: "Protocol Omega update successful. Security layers optimized.", likes: "8.9K", comments: "120", category: "TECH", location: "CORE" },
];

export default function FeedView({ onBack, onNavigate }) {
  ...
  const toggleLike = (id) => {
    if (likedItems.includes(id)) setLikedItems(likedItems.filter(i => i !== id));
    else { setLikedItems([...likedItems, id]); if (navigator.vibrate) navigator.vibrate(50); }
  };
  ...
  const handleShare = async (item) => {
    try { await navigator.share({ title: `Kiltikonet - ${item.user}`, text: item.description, url: window.location.href }); } catch (err) { /* share cancelled */ }
  };
        ...
        {FEED_DATA.map((item) => (
          <div key={item.id} className="h-full w-full snap-start relative flex items-center justify-center">
            <div className="absolute inset-0">
              <img src={item.content} alt={item.user} className="w-full h-full object-cover grayscale-[0.2] sepia-[0.1] brightness-[0.7]" referrerPolicy="no-referrer" />
            ...
            <div className="absolute right-4 sm:right-10 bottom-24 z-20 flex flex-col items-center gap-6 sm:gap-8">
              <div className="relative flex flex-col items-center gap-2">
                ...
                <motion.button whileTap={{ scale: 0.8 }} onClick={() => toggleFollow(item.user)} className="absolute -bottom-2 w-5 h-5 rounded-full flex items-center justify-center transition-all" style={{ background: followedUsers.includes(item.user) ? '#22c55e' : '#f2ca50' }}>
                  {followedUsers.includes(item.user) ? <ShieldCheck className="w-3 h-3 text-white" /> : <Plus className="w-3 h-3 text-black" />}
                </motion.button>
              </div>
              <motion.div whileTap={{ scale: 0.8 }} onClick={() => toggleLike(item.id)} className="flex flex-col items-center gap-1 cursor-pointer group">
                <Zap className={`w-8 h-8 sm:w-9 sm:h-9 transition-all ${likedItems.includes(item.id) ? "fill-[#f2ca50]" : "text-white group-hover:text-[#f2ca50]"}`} style={likedItems.includes(item.id) ? { color: '#f2ca50', filter: 'drop-shadow(0 0 10px #f2ca50)' } : {}} />
                <span className="text-[10px] sm:text-xs font-bold text-white tracking-widest">{item.likes}</span>
              </motion.div>
              <motion.div whileTap={{ scale: 0.8 }} onClick={() => setShowComments(item.id)} className="flex flex-col items-center gap-1 cursor-pointer group">
                <MessageSquare className="w-8 h-8 sm:w-9 sm:h-9 text-white group-hover:text-[#f2ca50] transition-colors" />
                <span className="text-[10px] sm:text-xs font-bold text-white tracking-widest">{item.comments}</span>
              </motion.div>
            ...
            <div className="absolute bottom-12 left-6 sm:left-10 z-20 max-w-[280px] sm:max-w-lg">
              <div className="flex items-center gap-3 mb-3">
                <span className="px-2 py-0.5 text-[8px] sm:text-[10px] tracking-widest rounded-sm uppercase" style={{ background: 'rgba(242,202,80,0.2)', color: '#f2ca50', border: '1px solid rgba(242,202,80,0.2)' }}>{item.category}</span>
                <span className="flex items-center gap-1 text-gray-400 text-[9px] sm:text-[11px] tracking-widest uppercase"><MapPin className="w-3 h-3" />{item.location}</span>
              </div>
              <h2 className="italic text-2xl sm:text-4xl text-white leading-tight mb-3 cursor-pointer hover:text-[#f2ca50] transition-colors" style={{ fontFamily: "'Noto Serif', serif", textShadow: '0 2px 10px rgba(0,0,0,0.5)' }}>@{item.user.replace(" ", "").toLowerCase()}</h2>
              <p className="text-gray-300 text-xs sm:text-base leading-relaxed opacity-90 mb-4">{item.description}</p>
            </div>

            <div className="absolute left-6 top-1/2 -translate-y-1/2 z-20 flex flex-col gap-4">
              {FEED_DATA.map((_, i) => (<div key={i} className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${activeIndex === i ? "scale-150" : "bg-white/20"}`} style={activeIndex === i ? { background: '#f2ca50', boxShadow: '0 0 8px #f2ca50' } : {}} />))}
            </div>
          </div>
        ))}
      </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-xl flex items-end sm:items-center justify-center p-4" onClick={() => setShowComments(null)}>
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="w-full max-w-lg overflow-hidden rounded-t-3xl sm:rounded-3xl" style={{ background: '#111', border: '1px solid rgba(255,255,255,0.1)' }} onClick={e => e.stopPropagation()}>
              ...
              <div className="p-4 bg-black/40 flex items-center gap-3" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <motion.button whileTap={{ scale: 0.9 }} className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: '#f2ca50', color: 'black' }}><Send className="w-4 h-4" /></motion.button>
              </div>
            </motion.div>
          </motion.div>
        )}
